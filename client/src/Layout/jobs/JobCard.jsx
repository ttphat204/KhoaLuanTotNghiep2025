...
import { FaHeart, FaClock, FaMapMarkerAlt, FaMoneyBillWave, FaBuilding, FaStar, FaEye, FaBriefcase, FaPaperPlane } from 'react-icons/fa';
...
const JobCard = ({ job }) => {
  ...
  return (
    <div
      className={`relative bg-white dark:bg-gray-800 rounded-2xl border transition-all duration-300 cursor-pointer overflow-hidden group h-[280px] perspective-1000 ${
        isHovered 
          ? 'border-blue-400 dark:border-blue-500 shadow-2xl shadow-blue-100 dark:shadow-blue-900/20' 
    ...
    >
      ...
      <div className={`relative w-full h-full transition-transform duration-700 transform-style-preserve-3d ${
        ...
        {/* Front Side - Always visible when not hovered */}
        <div className="absolute inset-0 w-full h-full backface-hidden bg-white dark:bg-gray-800 rounded-2xl">
          ...
          <div className="relative z-10 p-6 h-full flex flex-col">
            ...
            <div className="flex items-start justify-between mb-4 flex-shrink-0">
              <div className="flex-1 min-w-0">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2 line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-200 h-12">
                  {job.jobTitle}
                </h3>
            ...
            <div className="flex items-center gap-4 mb-4 flex-shrink-0">
              ...
              <div className="flex-1 space-y-2 min-w-0">
                <div className="flex items-center gap-2 text-sm">
                  <FaMoneyBillWave className="w-4 h-4 text-green-500 flex-shrink-0" />
                  <span className="font-bold text-green-600 truncate">{getSalary(job.salaryRange)}</span>
                </div>
            ...
            {/* Divider */}
            <div className="w-full h-px bg-gradient-to-r from-transparent via-gray-200 dark:via-gray-600 to-transparent mb-4 flex-shrink-0" />

            {/* Footer Section */}
            <div className="flex items-center justify-between mt-auto flex-shrink-0">
              <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                <FaClock className="w-3 h-3 flex-shrink-0" />
                <span className={`truncate ${getDeadline(job.applicationDeadline).includes('Hết hạn') ? 'text-red-500 font-medium' : ''}`}>
              ...
              <div className="flex items-center gap-2">
                {/* Urgent Badge */}
                {isUrgent() && (
                  <span className="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full animate-pulse flex items-center gap-1 flex-shrink-0">
        ...
        {/* Back Side - Only visible when hovered */}
        <div className="absolute inset-0 w-full h-full backface-hidden rotate-y-180 bg-gradient-to-br from-indigo-500 via-purple-600 to-blue-600 dark:from-indigo-700 dark:via-purple-700 dark:to-blue-700 text-white rounded-2xl">
          <div className="p-4 h-full flex flex-col">
            ...
            <div className="space-y-3 mb-4 flex-1">
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20">
                <div className="flex items-center justify-between">
                  <span className="text-indigo-200 text-sm">Mức lương</span>
                  <span className="font-semibold text-sm">{getSalary(job.salaryRange)}</span>
                </div>
              </div>
              
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20">
                <div className="flex items-center justify-between">
                  <span className="text-indigo-200 text-sm">Địa điểm</span>
                  <span className="font-semibold text-sm truncate">{getLocation(job.location)}</span>
                </div>
              </div>
            ...
            <div className="mt-auto">
              <button 
                className="w-full bg-white text-indigo-600 py-3 px-4 rounded-lg hover:bg-indigo-50 transition-all duration-300 font-semibold shadow-lg text-sm"
                onClick={(e) => {
                  e.stopPropagation();
                  handleCardClick();
                }}
